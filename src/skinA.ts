...
import { createQuery, getStore, getStoreByName } from "@datorama/akita";
import { async, firstValueFrom, interval, last, map, switchMap, tap } from "rxjs";
...
class SkinA extends HTMLElement {
  ...
  set miners(value: any) {
    console.log(value)
    this._miners = value;
    this.shadowRoot.querySelector('#miners').innerHTML = value.connectedMiners
  }
  get miners() {
    return this._miners;
  }
  ...
  constructor() {
   ...
   connectedCallback() {
    ...
    this.renderWorkersPartial();
      ...
      }), map(data => data.stats.reverse()[0])).subscribe(e => this.miners = e)
     statistics().subscribe(e => this.blocks = e)
